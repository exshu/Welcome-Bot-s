...
console.log(`eX / Welcome's | ${mesaj} | Bot : ${client.user.tag} | Bot ID : ${client.user.id} | Bot Sırası : 3 | Tarih : [${moment(new Date()).format("YYYY-MM-DD HH:mm:ss")}]`);
};

function exhata(hata) {
console.error(`eX / Welcome's | Botta Bir Hata Oluştu. | Bot Sırası : 3 | Tarih : [${moment(new Date()).format("YYYY-MM-DD HH:mm:ss")}]
Hata: ${hata}`);
};
...
client.login(process.env.BOTTOKEN3).then(x => exlog(`Bot Başarıyla Giriş Yaptı`)).catch(err => exhata(`Bot Giriş Yaparen Bir Hata Oluştu.
Hata : [${err.message}]`))
